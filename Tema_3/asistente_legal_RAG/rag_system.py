...
def initialize_rag_system():
    ...
    # Vector Store
    vectorestore = Chroma(
        embedding_function=OpenAIEmbeddings(model=EMBEDDING_MODEL, http_client=http_client),\
        persist_directory=CHROMA_DB_PATH
    )
    

    # Modelos
    llm_queries = ChatOpenAI(model=QUERY_MODEL, temperature=0, http_client=http_client)
    llm_generation = ChatOpenAI(model=GENERATION_MODEL, temperature=0, http_client=http_client)
